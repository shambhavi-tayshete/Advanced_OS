...
page_init(void)
{
	...
	uint32_t mpentry_num = PGNUM((MPENTRY_PADDR));
	//cprintf("\nPages:%x",pages);
	for (i = 0; i < npages; i++)
	{
		if(i==0 || i == mpentry_num)
		{
			pages[i].pp_ref = 1;
			continue;
		}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if((base + size) > MMIOLIM || (base + size) < base)
		panic("mapping above MMIOBASE");
	base = base+size;
	//cprintf("\nValue of base:%x",base);
	boot_map_region(kern_pgdir, retval, size, pa, PTE_PCD | PTE_PWT | PTE_W);
